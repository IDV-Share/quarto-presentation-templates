...
format:
    quarto-presentation-templates-pptx:
        ...
        fig-caption-dy: -10pt
        disable-postprocess: false # set this to true to disable postprocess features (allows quarto builds with no artifacts)
---
...
- To write marker keywords as plain text, escape the prefix: `\\LAYOUT::...` (same for `\\COLUMN::`, `\\BOX::`, `\\CAPTION::`, `\\VIDEO::`, ...).

## Hidden slide example {hidden=true}

- This slide is marked as hidden in Slide Show mode.
- You can also use `{hide=true}`.
...
This line is not formatted by the div style.

:::{style="font-size:12pt; font-weight:bold; font-family:Arial;"}
- This content inside `:::` is styled by `postprocess.py`.
- Supported attributes: `font-size`, `font-style`, `font-weight`, `font-family` (or via `style="..."`).
- Useful when Quarto/Pandoc cannot force these properties directly in PPTX.
- Nested bullet level 1
  - Nested bullet level 2
:::

This line is outside the div and keeps the default slide formatting.

:::{.Large}
This whole fenced block uses the `Large` class preset.
:::

:::{style="font-size:12pt; font-weight:bold; font-family:Arial;"}
Styled paragraph (no bullets) with inline style attributes.
:::
...
## Mermaid diagrams {layout="Title and Content"}

<!-- FIXME: wrong slide format selected by pandoc -->
```{mermaid}
%%| fig-width: 40%
flowchart LR
